...
| -------------------- | ------------------ | ------------------ | ------------------------- |
| `surface`            | `#EFEFEF`          | `#141414`          | Canvas: mesh, shell, rail |
...
| `textInverse`        | `#FFFFFF`          | `#141414`          | Text on solid and accent  |
| `solid`              | `#1C1C1C`          | `#EFEFEF`          | Primary button, mark chip |
| `solidHover`         | `#333333`          | `#FFFFFF`          | Solid hover               |
| `solidActive`        | `#000000`          | `#D2D2D2`          | Solid press               |
...
Saturated status hues are for degradation. An all-operational page should read as near-monochrome. `solid` is the console's one dark mass — the primary button and the product mark. Accent stays for links, focus, and maintenance status, never for a button fill.
...
| `sizeHeadline` / `lineHeadline`   | 30 / 36 — empty states only                             |
| `weightRegular`                   | 400                                                     |
| `weightMedium`                    | 500                                                     |
| `weightBold`                      | 600                                                     |
...
`sm` 6px (menu rows, badges, chips) · `md` 8px (buttons, inputs, surfaces) · `lg` 12px (the console panel) · `pill` 999px

## Shadow — `shadow.stylex.ts`

`subtle` separates a white surface from the canvas it sits on — the console panel, a secondary button, the active nav pill. `raised` for lifted objects. `overlay` for menus, dialogs, and toasts. Inlined consts (same `light-dark()` reason as color).
...
| `control`     | `focusWidth` 2px, `focusOffset` 2px, `heightSm` 28px, `heightMd` 32px, `heightLg` 40px, `sidebarWidth` 240px, `menuMaxWidth` 280px, `headerHeight` 52px, `markSize` 28px, `contentWidth` 880px, `contentWidthWide` 1320px |
